...
func TestCombatWithNoActiveSystem(t *testing.T) {
	// Save current system
	currentSystem := combat.GetActiveCombatSystem()
	currentName := combat.GetActiveCombatSystemName()
	
	// This test would need a way to clear the active system
	// For now, we'll just verify that combat works even if no delegated system
	
	user := createTestUser(3, "Hero", 100)
	require.NotNil(t, user, "Failed to create test user")
	
	mob := createTestMob(3, "Goblin", 60)
	require.NotNil(t, mob, "Failed to create test mob")
	
	// Attack should work regardless
	result := combat.AttackPlayerVsMob(user, mob)
	
	// Basic validation
	if result.Hit {
		assert.NotZero(t, result.DamageToTarget, "Hit but no damage")
	} else {
		assert.Zero(t, result.DamageToTarget, "Damage dealt on miss")
	}
	
	// Restore system if there was one
	if currentSystem != nil && currentName != "" {
		combat.SetActiveCombatSystem(currentName)
	}
}
