...
func (calc *RoundBasedCalculator) PowerRanking(attacker, defender *characters.Character) float64 {
	// Use the default implementation directly to avoid recursion
	// We can't call combat.PowerRanking as that would delegate back to us
	return powerRankingImpl(*attacker, *defender)
}

// powerRankingImpl is the default power ranking calculation
func powerRankingImpl(atkChar characters.Character, defChar characters.Character) float64 {
